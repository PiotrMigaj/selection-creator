...
const main = async () => {
  try {
    ...
    // Step 2: Get images from input directory
    console.log("Reading images from input directory...");
...
const uploadImagesToS3 = async (
  ...
  for (const file of files) {
    ...
    const objectKey = `${username}/${eventId}/selection/${file}`;
    ...
    uploadPromises.push(
      s3Client
        .send(command)
        .then(() => {
          console.log(`✔ Uploaded: ${file} to ${objectKey}`);
          uploadResults.push({
            fileName: file,
            objectKey: objectKey,
            contentType: contentType,
    ...
    for (const image of images) {
      ...
      promises.push(
        docClient
          .send(command)
          .then(() => {
            console.log(`✔ Created selection item record for: ${imageName}`);
          })
          .catch((err) => {
